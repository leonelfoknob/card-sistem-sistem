...
while continue_reading:
    ...
    '''if status_1 == MIFAREReader_1.MI_OK:
        ...
        if status_1 == MIFAREReader_1.MI_OK:
            ...
            query_1 = f"SELECT ismin,soyismin,sinif,ogrenci_no FROM ogreci_kayit WHERE (kart_no = %s)"
            result_1 = cur.execute(query_1,(ogrnci_kart_numarasi_giris,))
            print(result_1)
            print("Card read UID_1: %s" % uidToString(uid_1))
            if result_1 == 1:   
                query_giris = f"INSERT INTO ogreci_giris (ismin,soyisinm,sinif,ogrenci_no) SELECT ismin,soyismin,sinif,ogrenci_no FROM ogrenci_kayi WHERE (kart_no = %s)"
    ...
    if status_2 == MIFAREReader_2.MI_OK:
        ...
        if status_2 == MIFAREReader_2.MI_OK:
            ...
            print(ogrnci_kart_numarasi_cikis)
            '''query_2 = f"SELECT isim,soyisim,sinif,ogreci_no FROM ogrenci_kayit WHERE (kart_no = %s)"
